...
class GroupedQueryAttention(BaseAttention):
    ...
    def forward_with_cache(
        ...
        attends over the full ``past_len + q_len`` key/value set. Keys and
        values are cached after the per-group expansion, i.e. with
        ``num_heads`` entries, matching the shapes used by :meth:`forward`.
